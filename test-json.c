...
{
  fprintf(stderr, "test_json_cb__start_object\n");
...
{
  fprintf(stderr, "test_json_cb__end_object\n");
  TestInfo *t = callback_data;
  assert(*t->expected_callbacks_at == '}');
  t->expected_callbacks_at++;
  if (t->expected_callbacks_at[0] == ' ')
...
{
  fprintf(stderr, "test_json_cb__start_array\n");
  TestInfo *t = callback_data;
  assert(*t->expected_callbacks_at == '[');
  t->expected_callbacks_at++;
  if (t->expected_callbacks_at[0] == ' ')
...
{
  fprintf(stderr, "test_json_cb__end_array\n");
  TestInfo *t = callback_data;
  assert(*t->expected_callbacks_at == ']');
  t->expected_callbacks_at++;
  if (t->expected_callbacks_at[0] == ' ')
...
test_json_cb__object_key    (unsigned key_length,
                             ...
                             void *callback_data)
{
  fprintf(stderr, "test_json_cb__object_key(%s)\n", key);
  TestInfo *t = callback_data;
...
test_json_cb__number_value  (unsigned number_length,
                             ...
                             void *callback_data)
{
  fprintf(stderr, "test_json_cb__number_value(%s)\n", number);
  TestInfo *t = callback_data;
...
{
  TestInfo *t = callback_data;
  fprintf(stderr, "test_json_cb__string_value(%s)\n", string);
...
{
  TestInfo *t = callback_data;
  fprintf(stderr, "test_json_cb__boolean_value(%u)\n", boolean_value);
...
{
  TestInfo *t = callback_data;
  fprintf(stderr, "test_json_cb__null_value\n");
  assert(*t->expected_callbacks_at == 'N');
  t->expected_callbacks_at++;
  if (t->expected_callbacks_at[0] == ' ')
    t->expected_callbacks_at += 1;
